...
void MainWindow::on_newGameButton_clicked() { startNewGame(); }
...
void MainWindow::updateTileAppearance(int row, int col) {
  ...
  if (tileLabels.size() <= row || tileLabels[row].size() <= col ||
      !tileLabels[row][col]) {
    return;
  }
...
void MainWindow::animateTileMovement(QLabel *label, const QPoint &from,
                                     const QPoint &to) {
  QPropertyAnimation *animation =
      new QPropertyAnimation(label, "geometry", this);
  animation->setDuration(100);
  animation->setStartValue(
      QRect(from.x(), from.y(), label->width(), label->height()));
  animation->setEndValue(
      QRect(to.x(), to.y(), label->width(), label->height()));
...
void MainWindow::animateTileMerge(QLabel *label) {
  ...
  animation->setKeyValueAt(
      0.5, QRect(rect.x() + rect.width() / 8, rect.y() + rect.height() / 8,
